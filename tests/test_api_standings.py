...
async def test_driver_standings_2024() -> None:
    ...
    data = resp.json()
    assert len(data) >= 2  # At least VER and PER
    # Verify VER is P1 with 25 points
    ver_standing = next((s for s in data if s["driver_code"] == "VER"), None)
...
async def test_constructor_standings_2024() -> None:
    ...
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) >= 1  # At least RBR
    # Verify RBR is P1 with combined points (25 + 18 = 43)
    rbr_standing = next((s for s in data if s["team_ref"] == "red_bull_racing"), None)
    assert rbr_standing is not None
    assert rbr_standing["position"] == 1
    assert rbr_standing["points"] == 43.0  # VER 25 + PER 18
